...
var field = {};
var hasFocus = false;
//allows pixie dust to burst with every keystroke
...
function burst(intensity) {
  ...
  // left edge
  // orginal syntax in the if statement condition: input.value.length === 1
...
simulate("2d", {
  ...
  paint: function(particle) {
    ...
    this.paint.circle(p.x, p.y, s + 1.5, "rgba(231,244,255," + o * 0.25 + ")");
        ...
        return function(particle) {
          ...
          for (; i < l; i++) {
            ...
            if (
              ...
              Vector.distanceSquared(particles[i].position, particle.position) >
                range
            ) {
              continue;
            }
